...
import { Video, Send, Hash, Monitor, User, PhoneOff, Lock, AlertCircle, LogOut, Phone, X, Save, CheckCircle, Trash2, AlertTriangle, Crown, ShieldCheck, ShieldAlert, Bell, Paperclip, Smile, FileText, Download, Mic, MicOff, VideoOff } from 'lucide-react';
...
function App() {
  ...
  useEffect(() => {
    ...
    socket.on('offer', async (p) => {
      ...
      socket.emit('answer', { answer: a, room });
      setInCall(true);
    });
    socket.on('answer', async (p) => {
      await peerConnectionRef.current.setRemoteDescription(new RTCSessionDescription(p.answer));
    });
    ...
    socket.on('media_status', (data) => {
      if (data.type === 'mic') setRemoteMicEnabled(data.enabled);
  ...
  const startCall = async () => { setInCall(true); setIsRoomCreator(true); const s = await navigator.mediaDevices.getUserMedia({ video: true, audio: true }); if (myVideoRef.current) myVideoRef.current.srcObject = s; setMicEnabled(true); setVideoEnabled(true); setRemoteMicEnabled(true); setRemoteVideoEnabled(true); setTimeout(async () => { const pc = createPeerConnection(); const o = await pc.createOffer(); await pc.setLocalDescription(o); socket.emit('offer', { offer: o, room }); }, 100); };
  const joinCall = async () => { const s = await navigator.mediaDevices.getUserMedia({ video: true, audio: true }); if (myVideoRef.current) myVideoRef.current.srcObject = s; setMicEnabled(true); setVideoEnabled(true); setRemoteMicEnabled(true); setRemoteVideoEnabled(true); s.getTracks().forEach(t => { if (peerConnectionRef.current) peerConnectionRef.current.addTrack(t, s); }); const pc = peerConnectionRef.current; const o = await pc.createOffer(); await pc.setLocalDescription(o); socket.emit('offer', { offer: o, room }); };
  const endCall = () => { if (myVideoRef.current?.srcObject) { myVideoRef.current.srcObject.getTracks().forEach(t => t.stop()); myVideoRef.current.srcObject = null; } if (peerConnectionRef.current) { peerConnectionRef.current.close(); peerConnectionRef.current = null; } setInCall(false); setRemoteStream(null); setIsRoomCreator(false); setMicEnabled(true); setVideoEnabled(true); setRemoteMicEnabled(true); setRemoteVideoEnabled(true); };
  ...
  const toggleMic = () => {
    if (myVideoRef.current?.srcObject) {
      myVideoRef.current.srcObject.getAudioTracks().forEach(track => {
        ...
        socket.emit('media_status', { room, type: 'mic', enabled: track.enabled });
      });
    }
  };
  ...
  const toggleVideo = () => {
    if (myVideoRef.current?.srcObject) {
      myVideoRef.current.srcObject.getVideoTracks().forEach(track => {
        ...
        socket.emit('media_status', { room, type: 'video', enabled: track.enabled });
      });
    }
  };
    ...
    <div className="app-container">
      ...
      <div className="chat-window">
        <div className="chat-header">
          ...
          {!inCall ? (
            <button onClick={startCall} className="btn-primary"><Video size={18} /> Iniciar Vídeo</button>
          ) : (
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
              <div className="header-controls">
                <button onClick={toggleMic} className={`control-btn ${!micEnabled ? 'off' : ''}`} title={micEnabled ? "Mitar" : "Ativar Microfone"}>
                  {micEnabled ? <Mic size={18} /> : <MicOff size={18} />}
                </button>
                <button onClick={toggleVideo} className={`control-btn ${!videoEnabled ? 'off' : ''}`} title={videoEnabled ? "Desligar Câmera" : "Ligar Câmera"}>
                  {videoEnabled ? <Video size={18} /> : <VideoOff size={18} />}
                </button>
              </div>
              {!isRoomCreator && <button onClick={joinCall} className="btn-success"><Video size={18} /> Ligar Câmara</button>}
              <button onClick={hangUp} style={{ backgroundColor: '#EF4444', color: 'white', padding: '8px 15px', borderRadius: '8px', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', fontWeight: '600' }}><PhoneOff size={18} /> Sair</button>
            </div>
          )}
        </div>
        {inCall && (
          <div className="video-grid">
            <div className="video-card">
              ...
              <video ref={myVideoRef} autoPlay playsInline muted style={{ display: videoEnabled ? 'block' : 'none' }} />
              {!micEnabled && (
                <div className="status-overlay">
                  <MicOff size={20} color="#EF4444" />
                </div>
              )}
              <div className="user-label"> <User size={12} /> {username} (Eu)</div>
            </div>
            <div className="video-card" style={{ border: remoteStream ? '2px solid #10B981' : '2px dashed #444' }}>
              ...
              <video ref={remoteVideoRef} autoPlay playsInline muted style={{ display: remoteVideoEnabled ? 'block' : 'none' }} />
              {!remoteMicEnabled && remoteStream && (
                <div className="status-overlay">
                  <MicOff size={20} color="#EF4444" />
                </div>
              )}
